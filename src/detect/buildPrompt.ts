...
export function buildSystemPrompt(): string {
  return [
    ...
    '  - Always emit a suggested_key (a new candidate appropriate for the current context).',
    '  - Additionally, set duplicate_of_key whenever ANY key in <existing_keys> has the same user-visible meaning as the detected string — even if its wording is close but not identical (e.g. "Welcome" vs "Welcome back!", "Sign in" vs "Log in"). Be aggressive about spotting reuse opportunities; the consumer will verify the key exists and decide. duplicate_of_key MUST be an exact key from <existing_keys> (do not invent).',
    '',
    'Translations:',
